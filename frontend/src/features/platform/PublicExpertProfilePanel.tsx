...
export default function PublicExpertProfilePanel({
  ...
  const actionButtons = (
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
      ...
      >
        {!isLoggedIn ? 'Sign in to attach expert' : canUseProjectCart ? (inPlan ? 'In Planning' : 'Attach expert to Planning') : 'Open dashboard'}
      </Button>
  ...
  return (
    <Stack spacing={2.5}>
      <PageHeader
        ...
        description="Evaluate whether this specialist belongs in Planning."
      />
        ...
        <Surface>
          ...
          <Stack spacing={1.5}>
            ...
            <Button component={NextLink} href={canUseProjectCart ? PROJECT_START_PLAN_HREF : isLoggedIn ? '/dashboard' : '/login'} variant="contained" startIcon={<RocketLaunchOutlined />} sx={{ minHeight: 42 }}>
              {canUseProjectCart ? 'Planning' : isLoggedIn ? 'Open dashboard' : 'Sign in to start'}
            </Button>
          </Stack>
        </Surface>
      )}
