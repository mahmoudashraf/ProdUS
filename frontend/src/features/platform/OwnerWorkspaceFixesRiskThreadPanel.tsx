...
import type { CheckFixesResponse, ScannerRiskState, ScannerRiskSummary, ScannerRiskThread } from './types';

interface OwnerWorkspaceFixesRiskThreadPanelProps {
...
const activeStates: ScannerRiskState[] = ['NEW', 'STILL_OPEN', 'RETURNED', 'READY_TO_CHECK', 'NEEDS_PROOF', 'INCOMPLETE_CHECK'];
...
export default function OwnerWorkspaceFixesRiskThreadPanel({
  ...
  lastCheck,
  onCheckFixes,
}: OwnerWorkspaceFixesRiskThreadPanelProps) {
  ...
  return (
    <Surface sx={{ mb: 1.5, background: 'linear-gradient(135deg, #ffffff 0%, #f7fbff 55%, #fffaf0 100%)' }}>
      <Stack spacing={1.5}>
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.25} justifyContent="space-between" alignItems={{ xs: 'stretch', md: 'flex-start' }}>
          <Box>
            <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap>
              ...
              <PastelChip label={`${activeRisks.length} current`} accent={activeRisks.length ? appleColors.amber : appleColors.green} bg={activeRisks.length ? '#fff4dc' : '#e7f8ee'} />
              {fixedCount > 0 && <PastelChip label={`${fixedCount} fixed`} accent={appleColors.green} bg="#e7f8ee" />}
              {returnedCount > 0 && <PastelChip label={`${returnedCount} returned`} accent={appleColors.red} bg="#ffe9e4" />}
            </Stack>
            ...
            <Typography color="text.secondary" sx={{ mt: 0.45, maxWidth: 780, lineHeight: 1.6 }}>
              These are product scan risks that have become workspace work. Check fixes reruns the smallest relevant scanner set for these risks.
            </Typography>
          </Box>
          <Button
            variant="contained"
          ...
          >
            {isChecking ? 'Checking fixes...' : readyCount ? 'Check ready fixes' : 'Check fixes'}
          </Button>
        </Stack>

        {lastCheck && (
          <Alert severity={lastCheck.queuedRuns.length ? 'success' : 'warning'}>
            {lastCheck.preview.summary} {lastCheck.preview.tools.length ? `Tools: ${lastCheck.preview.tools.join(', ')}.` : ''}
            {lastCheck.skippedTargets.length ? ` ${lastCheck.skippedTargets.length} target${lastCheck.skippedTargets.length === 1 ? '' : 's'} need attention.` : ''}
          </Alert>
        )}
        ...
        {isLoading ? (
          ...
          <Box sx={{ border: '1px dashed', borderColor: '#c9d8ea', borderRadius: 1, p: 1.25, bgcolor: '#fff' }}>
            <Typography variant="subtitle1">No scan risks selected for this workspace yet.</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.4, lineHeight: 1.55 }}>
              Add product scanner risks to this workspace when they should become work for services, people, milestones, and proof.
            </Typography>
          </Box>
        )}
      </Stack>
...
function RiskThreadCard({ risk }: { risk: ScannerRiskThread }) {
  const accent = risk.currentState === 'FIXED_BY_LATEST_SCAN'
    ? appleColors.green
    : risk.currentState === 'RETURNED'
      ...
      : risk.currentState === 'READY_TO_CHECK'
        ? appleColors.blue
        : appleColors.amber;
  const Icon = risk.currentState === 'FIXED_BY_LATEST_SCAN'
  ...
  return (
    <Box sx={{ p: 1.2, border: '1px solid', borderColor: `${accent}2e`, borderRadius: 1, bgcolor: '#fff', minHeight: 178 }}>
      <Stack spacing={1}>
