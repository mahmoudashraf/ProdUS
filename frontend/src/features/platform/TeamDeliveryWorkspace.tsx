      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '1fr 330px' }, gap: 2.5 }}>
        <Stack spacing={2.5}>
          ...
          <PlatformAssistantCard
            ...
            prompt={`Do not call tools for this answer. Use only the facts in this prompt and the supplied safe summaries. Review delivery evidence for ${selectedWorkspace?.name || 'the selected delivery'} from the team perspective. Delivery status is ${selectedWorkspace?.status || 'not selected'}, product is ${selectedWorkspaceProduct?.name || 'not available'}, visible checkpoints are ${(milestones.data || []).slice(0, 5).map((milestone) => `${milestone.title} (${milestone.status})`).join('; ') || 'none'}, attachments count is ${attachments.data?.length || 0}, support risks are ${teamSupport.slice(0, 4).map((request) => `${request.title} (${request.priority}, ${request.status})`).join('; ') || 'none'}. Focus on missing proof, blocked deliverables, owner-facing evidence to add, and safe next team actions. Do not approve production readiness.`}
