      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '1fr 330px' }, gap: 2.5 }}>
        <Stack spacing={2.5}>
          ...
          <PlatformAssistantCard
            ...
            prompt={`Do not call tools for this answer. Use only the facts in this prompt and the supplied safe summaries. Create a team-facing delivery proof note for ${selectedWorkspace?.name || 'the selected delivery'}. Delivery status is ${selectedWorkspace?.status || 'not selected'}, product is ${selectedWorkspaceProduct?.name || 'not available'}, visible stage count is ${(milestones.data || []).length}, blocked or submitted stages count is ${(milestones.data || []).filter((milestone) => ['BLOCKED', 'SUBMITTED', 'IN_REVIEW'].includes(milestone.status)).length}, attachments count is ${attachments.data?.length || 0}, support risk count is ${teamSupport.length}. Focus on missing proof, blocked deliverables, owner-facing proof to add, and safe next team actions. Do not approve production readiness.`}
