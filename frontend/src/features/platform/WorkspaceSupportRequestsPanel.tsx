...
interface WorkspaceSupportRequestsPanelProps {
...
}: WorkspaceSupportRequestsPanelProps) {
  return (
    <Surface>
      ...
      <Stack spacing={1.25}>
        {supportList.length ? supportList.map((request) => (
          <Box key={request.id} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, p: 1.25 }}>
            ...
            <Typography variant="caption" color="text.secondary">{request.team?.name || 'Unassigned'} · {formatLabel(request.priority)}</Typography>
