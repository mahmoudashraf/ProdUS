...
import { CancelOutlined, CloudDoneOutlined, DeleteSweepOutlined, RefreshOutlined, ReplayOutlined, SecurityOutlined } from '@mui/icons-material';
...
import { ScanRun, ScannerAdminHealth, ScannerConnectorInstallation, ScannerRetentionRun } from './types';
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '1fr 1fr' }, gap: 2 }}>
            <Surface>
              <Stack direction={{ xs: 'column', md: 'row' }} spacing={1} alignItems={{ md: 'center' }} justifyContent="space-between" sx={{ mb: 1.5 }}>
                ...
                <PastelChip label={`${connectorInstallations.data?.length || 0} installed`} accent={appleColors.cyan} bg="#e4f9fd" />
              </Stack>
              {connectorInstallations.data?.length ? (
                <Stack spacing={1}>
                  {connectorInstallations.data.slice(0, 6).map((installation) => (
            ...
            <Surface>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ mb: 1.5 }}>
                <Button
                  ...
                  onClick={() => retentionRun.mutate(true)}
                  disabled={retentionRun.isPending}
                  sx={{ minHeight: 42 }}
                >
                  Dry Run Retention
                </Button>
                <Button
                  ...
                  color="warning"
                  startIcon={<DeleteSweepOutlined />}
                  onClick={() => retentionRun.mutate(false)}
                  disabled={retentionRun.isPending}
                  sx={{ minHeight: 42 }}
                >
                  Execute Cleanup
              ...
              {retentionRun.data ? (
                <Box sx={{ p: 1.25, border: '1px solid', borderColor: appleColors.line, borderRadius: 1, bgcolor: '#fff' }}>
                  <Stack direction="row" justifyContent="space-between" spacing={1}>
                    <Typography sx={{ fontWeight: 900 }}>{retentionRun.data.dryRun ? 'Dry run complete' : 'Cleanup complete'}</Typography>
                    <PastelChip label={`${retentionRun.data.candidateCount} candidates`} accent={retentionRun.data.candidateCount ? appleColors.amber : appleColors.green} bg={retentionRun.data.candidateCount ? '#fff4dc' : '#e7f8ee'} />
                  </Stack>
                  {retentionRun.data.candidates.slice(0, 5).map((candidate) => (
