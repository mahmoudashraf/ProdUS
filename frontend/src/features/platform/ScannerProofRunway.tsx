  ...
  const steps = [
    {
      ...
      detail: sourceCount ? `${sourceCount} authorized source${sourceCount === 1 ? '' : 's'} attached.` : 'Attach the repository, runtime URL, or scanner export first.',
    ...
    },
    {
      ...
      detail: `${latestCoveredTools}/${totalTools} scanner tools have a latest result.`,
      complete: runComplete,
      accent: runComplete ? appleColors.green : activeScanRun ? appleColors.purple : appleColors.blue,
      action: (
        <Tooltip title={blockedReason || 'Queue the complete repository, container, and runtime proof suite.'}>
          <span>
            <Button
              size="small"
            ...
            >
              {isStartingFullSuite ? 'Starting...' : runComplete ? 'Run again' : 'Run suite'}
            </Button>
          </span>
        </Tooltip>
      ),
    },
    {
      ...
      detail: hasPriorityFindings ? `${criticalCount} critical, ${highCount} high, ${openFindingCount} open.` : 'No priority blocker is visible in the latest proof.',
    ...
    },
    {
      ...
      detail: evidenceCount ? `${evidenceCount} stored proof item${evidenceCount === 1 ? '' : 's'} can support the owner report.` : 'Proof export becomes useful after evidence is attached.',
  ...
  return (
    <Box>
      <SectionTitle
        ...
        action={
          <PastelChip
            label={`${normalizedFindingCount} normalized findings`}
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '220px repeat(3, minmax(0, 1fr))' }, gap: 1.5, mb: 2 }}>
        ...
        <MetricTile
          ...
          value={sourceCount}
          detail="Repo, runtime, CI, or imports"
      ...
      <Box sx={{ p: 1.5, borderRadius: 1, border: '1px solid', borderColor: '#dbeafe', bgcolor: '#fbfdff', mb: 2 }}>
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.25} justifyContent="space-between" alignItems={{ md: 'center' }} sx={{ mb: 1.25 }}>
          <Box>
            <Typography sx={{ fontWeight: 950 }}>Owner proof runbook</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.35, lineHeight: 1.6 }}>
              Follow this short path when a founder asks whether the product has enough evidence for launch, beta, or investor review.
            </Typography>
          </Box>
          {blockedReason && (
