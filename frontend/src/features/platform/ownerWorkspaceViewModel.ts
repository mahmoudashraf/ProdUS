...
} from './ownerWorkspaceStartPlanState';
import {
...
export function buildOwnerWorkspaceViewModel({
  ...
  const productProposals = proposals.filter((proposal) => proposal.packageInstance?.productProfile?.id === selectedProduct?.id);
  ...
  const latestScannerDiagnosis = diagnoses.find((diagnosis) => diagnosis.diagnosisSource === 'SCANNER_READINESS');
  const scannerCounts = scannerSummary?.counts;
  const scannerReadiness = scannerSummary?.readinessScore ?? (scannerCounts?.total ? 72 : 100);
  const scannerOpenFindings = (scannerSummary?.findings || []).filter((finding) => ['NEW', 'OPEN', 'REGRESSED'].includes(finding.status));
  const hasCompletedScannerRun = !!scannerSummary?.recentRuns.some((run) => run.status === 'COMPLETED');
  ...
  const hasLaunchEvidenceContext = !!scannerSummary || !!latestDiagnosis || !!launchReadinessReport || hasCompletedScannerRun || latestMappedToolFindings > 0;
  ...
  const topRecommendedServiceName = scannerMappedServices[0] || selectedPackage?.name || startPlan.cartServiceItems[0]?.serviceModule.name || '';
  ...
  const ownerActionGroups = buildOwnerActionGroups({
    topOwnerRisks,
    weekItems: scannerMappedServices.length ? scannerMappedServices : startPlan.cartStartGaps.map((gap) => gap.title),
  ...
  const selectedMilestone = milestones.find((milestone) => milestone.status === 'BLOCKED')
    || milestones.find((milestone) => milestone.status === 'SUBMITTED' || milestone.status === 'IN_PROGRESS')
    || milestones[0];
