...
const findingReviewPrompt = (
  ...
  `Do not call write actions for this answer. Review scanner finding ${finding.id} for ${product.name}. Finding details: title "${finding.title}", severity ${finding.severity}, status ${finding.status}, affected area "${finding.readinessArea || finding.affectedComponent || 'not specified'}", source rule "${finding.sourceRuleId || finding.sourceTool}", description "${finding.description}", business risk "${finding.businessRisk || 'not mapped'}", required evidence "${finding.evidenceRequired || 'not mapped'}", recommended service "${finding.recommendedModule?.name || 'not mapped'}", linked evidence count ${linkedEvidenceCount}. Explain likely impact, what evidence is needed to resolve or accept risk, and which productization service or milestone should own follow-up. Use these provided details directly. Do not include raw artifact contents.`;
