...
const CardStyle3 = () => {
  ...
  // Get profile cards data from Context API
  const profileCards = userContext.state.profileCards;
  ...
  const { execute: loadUsers } = useAsyncOperation(
    async () => {
      await userContext.getProfileCards();
    ...
    },
    {
      ...
      onError: () => {
        notificationContext.showNotification({
          message: 'Failed to load profile cards',
          variant: 'error',
  ...
  const { execute: searchUsers } = useAsyncOperation(
    async (query: string) => {
      if (query) {
        await userContext.filterProfileCards(query);
      } else {
        await userContext.getProfileCards();
      }
      return true;
...
export default withErrorBoundary(CardStyle3);
