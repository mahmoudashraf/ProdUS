...
const PlanList = ({ plan, view, priceFlag }: { plan: plan; view: number; priceFlag: string }) => {
  ...
  return (
    <Grid size={{ xs: 12 }} sx={{ display: view !== plan.id ? { xs: 'none', sm: 'block' } : 'block' }}>
      <FeatureContentWrapper>
        {plan.popular && (
          <PopularBadgeWrapper sx={{ transform: rtlLayout ? 'rotate(316deg)' : 'rotate(45deg)' }}>
            Popular
          </PopularBadgeWrapper>
        )}
...
const ListItem = ({ item, index, view }: { item: number; index: number; view: number }) => (
  <Grid size={4} sx={{ display: view !== index + 1 ? { xs: 'none', sm: 'block' } : 'block' }}>
    {item === 1 && (
      <Box sx={{ px: 3, py: 1.5 }}>
...
const OrderButton = ({
  ...
  <Grid size={{ xs: 12 }} sx={{ display: view !== index ? { xs: 'none', sm: 'block' } : 'block' }}>
    <FeatureContentWrapper>
      <Button
...
const Price2 = () => {
  ...
  const handleChange = (_event: React.SyntheticEvent, newView: number) => {
    if (newView !== null) {
      setView(newView);
    }
  };
