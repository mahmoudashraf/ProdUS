...
const filter: (options: FilmFilter[], state: FilterOptionsState<FilmFilter>) => FilmFilter[] =
  createFilterOptions();
...
const FrmAutocomplete = () => {
  ...
  const defaultProps = {
    options: top100Films.filter(Boolean) as FilmFilter[],
    getOptionLabel: (option: FilmFilter) => option?.title ?? '',
  };

  const flatProps = {
    options: top100Films.map(option => option.title).filter(Boolean) as string[],
  };

  const optionsGroup = top100Films.map(option => {
    const first = (option.title?.[0] || '').toUpperCase();
    const firstLetter = /[0-9]/.test(first) ? '0-9' : first || '0-9';
    return {
      firstLetter,
      ...option,
    } as { title: string; year?: number; firstLetter: string };
  });

  return (
    <MainCard
      title="Autocomplete"
      secondary={<SecondaryAction link="https://next.material-ui.com/components/autocomplete/" />}
    >
            ...
            <Grid size={{ xs: 12 }}>
              <SubCard title="Creatable">
                <Autocomplete
                  ...
                  options={top100Films.filter(Boolean) as any[]}
            ...
            <Grid container direction="column" spacing={2}>
              ...
              <Grid size={{ xs: 12 }}>
                <Autocomplete
                  ...
                  id="controlled-demo"
                  value={valueControl}
                  onChange={(_event: any, newValue: FilmFilter | null) => {
