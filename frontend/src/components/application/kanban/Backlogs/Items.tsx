...
const Items = ({ itemId, index }: Props) => {
  ...
  const { columns, items, profiles, userStory } = kanbanContext.state;

  const item = items.find((data: KanbanItem) => data.id === itemId);
  if (!item) return null;
  
  const itemColumn = columns.find(column => column.itemIds.includes(item.id));
  const itemProfile = profiles.find(profile => profile.id === item.assign);

  const handlerDetails = () => {
    try {
      kanbanContext.selectItem(itemId);
    } catch (error) {
