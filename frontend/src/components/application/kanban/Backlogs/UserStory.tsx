...
const getDropWrapper = (isDraggingOver: boolean, theme: Theme) => {
  const bgcolor =
    theme.palette.mode === 'dark' ? theme.palette.background.default : theme.palette.grey[100] + 80;
  return {
...
const UserStory = ({ story, index }: Props) => {
  ...
  const { columns, profiles, userStory, userStoryOrder } = kanbanContext.state;
  const [open, setOpen] = React.useState(false);

  const storyColumn = columns.filter((column: any) => column.id === story.columnId)[0];
  const storyProfile = profiles.filter((profile: any) => profile.id === story.assign)[0];
        ...
        {(provided, snapshot) => (
          <>
            <TableRow
              ...
              sx={getDragWrapper(snapshot.isDragging, theme, open) as any}
            >
              ...
              <TableCell sx={{ width: 85, minWidth: 85, textTransform: 'capitalize' }}>
                {story.priority}
              </TableCell>
